...
| GitHub Actions | 100% | 三 job 编排: validate(免密, 含 provider 接线探针) → redteam+gate → timing; push/schedule/dispatch 三种触发 |
| 真正 RedTeam 执行 | 95% | 原工作流的 `--dry-run` 冒烟是无效参数(被 `\|\| true` 掩盖); 定向套件 target 已改为经验证的 `openai:chat + apiBaseUrl` 写法; 配置 secret 后即可真实执行 |
